...
# -------------------------
# 6. COMMENTED OUT: Train the Model on All Clauses and Annotations
# -------------------------
...
# -------------------------
# 7. Load the Pre-trained Model and Run Inference
# -------------------------
...
train_acc = evaluate_model(model, data_list)
print("Node classification accuracy (Train): {:.2f}%".format(train_acc * 100))

# Load test data from JSON
test_json_path = "/DATA/sujit_2021cs35/nlp_project/code/task 2/train_set.json"
test_clauses = []
test_labels = []
...
def compute_metrics(model, data_list):
    model.eval()
    all_predictions = []
    all_targets = []
    ...
    class_names = ["emotion_clause", "cause_clause", "neutral"]
...
print("\n--- Detailed Metrics ---")
report, conf_matrix = compute_metrics(model, test_data_list)
...
def visualize_graph(clauses, true_labels, predictions, index, save_dir):
    """
    Generate a visualization of the graph with nodes colored by true labels and predictions
    
    Args:
        clauses: List of clause texts
        true_labels: List of true label indices
        predictions: List of predicted label indices
        index: Index of this graph in the dataset (for filename)
    ...
    # Map indices to label names for better readability
    label_names = ["emotion", "cause", "neutral"]
    color_map = ["#FF9999", "#99FF99", "#9999FF"]  # Red, Green, Blue
    
    # Add nodes with attributes
    for i, clause in enumerate(clauses):
        # Truncate long clauses for display
        display_text = clause[:30] + "..." if len(clause) > 30 else clause
        
        # Get true and predicted labels
        true_idx = true_labels[i] if i < len(true_labels) else 2  # Default to neutral
        pred_idx = predictions[i] if i < len(predictions) else 2  # Default to neutral
        ...
        G.add_node(i, 
                  ...
                  true_color=color_map[true_idx],
                  pred_color=color_map[pred_idx])
    ...
    # Draw nodes with true label colors (circle)
    nx.draw_networkx_nodes(G, pos, 
                         node_color=[G.nodes[n]['true_color'] for n in G.nodes],
                         node_size=500,
                         alpha=0.8)
    
    # Draw node edges in black
    nx.draw_networkx_edges(G, pos, width=1.0, alpha=0.5)
    
    # Draw node labels (index + text)
    labels = {n: f"{n}: {G.nodes[n]['text']}" for n in G.nodes}
    nx.draw_networkx_labels(G, pos, labels=labels, font_size=8)
    
    # Add a title and legend
    plt.title(f"Graph #{index}: True labels (node color) vs Predicted labels (node border)")
    
    # Create legend for node colors
    legend_elements = [plt.Line2D([0], [0], marker='o', color='w', 
                                 markerfacecolor=color, markersize=10, label=label)
                      for label, color in zip(label_names, color_map)]
    ...
    # Save the figure
    plt.savefig(os.path.join(save_dir, f"graph_{index}.png"), dpi=150)
    plt.close()
    
    print(f"Saved visualization for graph #{index}")

# Visualize a sample of test graphs with their predictions
print("\nGenerating graph visualizations...")
visualization_dir = "/DATA/sujit_2021cs35/nlp_project/code/task 3/graph_visualizations"
os.makedirs(visualization_dir, exist_ok=True)

# Visualize up to 10 random test graphs
import random
sample_indices = random.sample(range(len(test_data_list)), min(10, len(test_data_list)))

for idx in sample_indices:
    data = test_data_list[idx]
    pred = test_predictions[idx]
    
    # Get the clauses for this graph
    clauses = test_clauses[idx]
    
    # Get true labels
    true_labels = data.y.cpu().numpy()
    
    # Visualize the graph
    visualize_graph(clauses, true_labels, pred, idx, visualization_dir)
